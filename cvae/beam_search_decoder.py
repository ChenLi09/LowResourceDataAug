...
class BeamSearchDecoder_topk(BeamSearchDecoderMixin, decoder.Decoder):
  ...
  """BeamSearch sampling decoder.
    ...
    **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
    ...
    when computing
    s (https://arxiv.org/pdf/1609.08144.pdf). It encourages
  ...
  def __init__(self,
    ...
    """
    super(BeamSearchDecoder_topk, self).__init__(
        cell,
        beam_width,
...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
  ...
  Returns:
    A new beam state.
  """
  # beam_state_cpy = copy.deepcopy(beam_state.log_probs)
  ...
  top_k_probs, indices = nn_ops.top_k(logits, k=next_beam_size)
  kth = tf.reduce_mean(top_k_probs, axis=2, keepdims=True)
  top_k = tf.greater_equal(logits, kth)
  mask = tf.cast(top_k, dtype=tf.float32)
  v = tf.multiply(logits, mask)
  sum = tf.reduce_sum(v, axis=2)
  recip = tf.reciprocal(sum)
  v = tf.transpose(v, [0, 2, 1])
  recip = tf.expand_dims(recip, axis=1)
  norms = tf.multiply(v, recip)
  norms = tf.transpose(norms, [0, 2, 1])
  _ = nn_ops.log_softmax(norms)
  _1 = _mask_probs(_, end_token, previously_finished)
  step_log_probs = nn_ops.log_softmax(norms)
